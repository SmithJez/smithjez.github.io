...
function GenerateDungeon(url, url2, url3, url4, url5, url6, dunType) {
  ...
  for (i = 0; i < displayArray.length; i++) {
    ...
    if (enumDunType == dunType.toLowerCase()) {
      golemArray = displayArray[i][11];
    }
  ...
  for (i = 0; i < golemArray.length; i++) {
    ...
    CreateSkillDom(stringMap, div_skill_col_holder, boss_passive_skill_effect, boss_passive_skill_effect_icon, "Passive Skill" )
    goog.dom.appendChild(div_skill_col_holder, goog.dom.createDom('br',""));
    CreateSkillDom(stringMap, div_skill_col_holder, boss_active_skill_effect, boss_active_skill_effect_icon, "Active Skill" )
    goog.dom.appendChild(div_skill_col_holder, goog.dom.createDom('br',""));
    CreateSkillDom(stringMap, div_skill_col_holder, boss_cond_skill_effect, boss_cond_skill_effect_icon, "Conditional Skill" )
    // END SKILL


    // var div_skill_col_holder = goog.dom.createDom('div', "col", "");
    // goog.dom.appendChild(div_skill_col, div_skill_col_holder);
    ...
    for(kk = 0; kk < boss_skill_list.length; kk++) {
      ...
      CreateSkillDom(stringMap, div_skill_col_holder, status_effect, status_effect_icon, "Boss Skill" )
...
function SetSkillDesc (ori_desc, m) {
  ...
  // var text3 = text2.replace(p2, function(match, text, urlId) {
  //   return `${text}`;
  // }) ;

  // console.log(text3);

  text3 = text3.replace("{0}", arg).replace("{1}", arg2).replace("{2}", arg3);
...
function CreateStatDom (stringMap, group_i_1, monBossObj, stageMonBoss, bossStat, color ) {
  ...
  var stat_list = ["Hp", "Attack", "Defense", "Recovery", "Crit Dmg", "Crit Rate", "Resist"];
  ...
  for (jj = 1; jj < bossStat.length; jj++) {
    ...
    var div_lv_left = goog.dom.createDom('div', "stat_col", stat_list[jj - 1]);
