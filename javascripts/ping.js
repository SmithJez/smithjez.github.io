...
function readUrl(url, url2, url3, url4, url5, url6, dunType) {
  ...
  for (i = 0; i < golemArray.length; i++) {
    ...
    var img = goog.dom.createDom('img', "monster_icon")
    img.setAttribute('src', '/img/' + monBossObj[29] + '.png')
    // var td = goog.dom.createDom('td', undefined, img)
    var tdName = goog.dom.createDom('td', {
      'align': 'center'
    }, stringMap[monBossObj[1]])
    var tdBossLv = goog.dom.createDom('td', {
      'align': 'center'
    }, stageMonBoss[2].toString())
    ...
    for (j = 1; j < bossStat.length; j++) {
      var bossData = goog.dom.createDom('td', {
        'align': 'center'
      }, bossStat[j].toString())
    ...
    if (fixedMon.length == 0) {
      ...
      for (x = 0; x < stageMonSize; x++) {
        ...
        line2.setAttribute('bgColor', "#19618e")
        var img = goog.dom.createDom('img', "monster_icon")
        img.setAttribute('src', '/img/' + monMinionBossObj[29] + '.png')
        var td = goog.dom.createDom('td', undefined, img)
        var tdName = goog.dom.createDom('td', {
          'align': 'center'
        }, stringMap[monMinionBossObj[1]])
        var tdMinionBossLv = goog.dom.createDom('td', {
          'align': 'center'
        }, stageMonMinionBoss[2].toString())
    ...
    for (j = 0; j < mMonGroup[1].length; j++) {
      ...
      var img = goog.dom.createDom('img', "monster_icon")
      img.setAttribute('src', '/img/' + monObj[29] + '.png')
      var td = goog.dom.createDom('td', undefined, img)
      var tdName = goog.dom.createDom('td', {
        'align': 'center'
      }, stringMap[monObj[1]])
      ...
      for (k = 1; k < monStat.length; k++) {
        var monData = goog.dom.createDom('td', {
          'align': 'center'
        }, monStat[k].toString())
